...
void CPlanarMirrorShader::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, void* pContext)
{
	...
	m_ppObjects[0]->SetPosition(XMFLOAT3(890.0f, pTerrain->GetHeight(890.0f, 1190.0f) + 20.0f, 1190.0f));
	...
	m_ppObjects[1]->SetPosition(XMFLOAT3(920.0f, pTerrain->GetHeight(920.0f, 1200.0f) + 20.0f, 1200.0f));
	m_ppObjects[1]->SetScale(3.0f, 3.0f, 3.0f);
	m_ppObjects[1]->Rotate(0.0f, 90.0f, 0.0f);

	m_ppObjects[2] = new CSuperCobraObject(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature);
	m_ppObjects[2]->SetChild(pSuperCobraModel);
	pSuperCobraModel->AddRef();
	m_ppObjects[2]->PrepareAnimate();
	m_ppObjects[2]->SetPosition(XMFLOAT3(870.0f, pTerrain->GetHeight(870.0f, 1150.0f) + 40.0f, 1150.0f));
	m_ppObjects[2]->SetScale(3.0f, 3.0f, 3.0f);
	m_ppObjects[2]->Rotate(0.0f, 90.0f, 0.0f);

	m_ppObjects[3] = new CGunshipObject(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature);
	m_ppObjects[3]->SetChild(pGunshipModel);
	pGunshipModel->AddRef();
	m_ppObjects[3]->PrepareAnimate();
	m_ppObjects[3]->SetPosition(XMFLOAT3(910.0f, pTerrain->GetHeight(910.0f, 1190.0f) + 60.0f, 1190.0f));
	...
	m_pWallObject->SetPosition(XMFLOAT3(920.0f, pTerrain->GetHeight(920.0f, 1115.0f), 1115.0f));

	m_pMirrorObject = new CMirror(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature);
	m_pMirrorObject->SetPosition(XMFLOAT3(920.0f, pTerrain->GetHeight(920.0f, 1120.05f), 1120.05f));
...
{
	UINT ncbElementBytes = ((sizeof(VS_CB_REFLECTION_MATRIX) + 255) & ~255); //256의 배수
	m_pd3dcbReflection = ::CreateBufferResource(pd3dDevice, pd3dCommandList, NULL, ncbElementBytes, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, NULL);
...
D3D12_SHADER_BYTECODE CTerrainTessellationShader::CreateVertexShader(int nPipelineState)
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "VSTerrainTessellation", "vs_5_1", &m_pd3dVertexShaderBlob));
}

D3D12_SHADER_BYTECODE CTerrainTessellationShader::CreatePixelShader(int nPipelineState)
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "PSTerrainTessellation", "ps_5_1", &m_pd3dPixelShaderBlob));
}

D3D12_SHADER_BYTECODE CTerrainTessellationShader::CreateDomainShader(int nPipelineState)
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "DSTerrainTessellation", "ds_5_1", &m_pd3dDomainShaderBlob));
}

D3D12_SHADER_BYTECODE CTerrainTessellationShader::CreateHullShader(int nPipelineState)
{
